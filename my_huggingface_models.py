...
class HuggingfaceModel(BaseModel):
        ...
        if 'llama' in model_name.lower():
            ...
            if ('7b' in model_name or '13b' in model_name) or eightbit:
                self.model = AutoModelForCausalLM.from_pretrained(
                    f"{base}/{model_name}", device_map="cuda:1", # 修改device
    ...
    def get_token_log_likelihoods(self, input_sentence):
        # 预处理输入
        ...
        for i in range(inputs['input_ids'].shape[1]):
            ...
            log_prob = logits[0, i, token_id].item()  # 获取该 token 的 log likelihood
    ...
    def get_token_log_likelihoods_dic(self, input_sentence):
        ...
        end_log_prob = token_probs[end_token_id].item()
        log_likelihoods.append(('[EOS]', end_log_prob))
